...
- **Short post-wake commands never dropped (pre-roll → VAD)** — a command spoken right after the wake word ("Ultron say hello") lands in the **cold pre-roll** snapshot that is captured *before* the live capture loop starts. Previously the loop's VAD only ever saw the trailing silence, so `speech_seen` never latched and the turn was discarded as an empty capture even though the wake word fired perfectly — Ultron stayed silent and you had to repeat yourself (the "said it eight times before he answered" freeze). The capture now feeds the pre-roll through the VAD first (it self-slices into 512-sample windows), so speech inside it latches the capture open and the buffer that already holds the command is transcribed. As a guard — now that a bare "Ultron" also registers as speech — an utterance that is *only* the wake word (or a mis-heard wake remnant) with nothing after it stands down silently instead of routing, checked on the **raw** transcript before normalization (which would otherwise turn a bare "Tron" into the callout "drone" and fire a spurious turn)
