...
- **Addressee confidence (follow-up window)** — in the wake-free ~30 s window after Kenning speaks, each utterance is scored by a **calibrated log-odds fusion** instead of a flat zero-shot verdict. Graded lexical/structural features — leading wake word, imperative/command shape, factual question (strong **+**); third-person mention ("Ultron said …"), subject-pronoun / answer-particle / phone openers, narration (strong **−**) — combine with the zero-shot model's **real** P(YES) and a recency prior, then `sigmoid →` a **cost-asymmetric threshold** (`KENNING_ADDRESSING_TAU`, raise it on a live stream). The model is consulted *only* in the undecided band, so "Ultron, show me the stop button" is ADDRESSED with **zero model latency** while "yeah I dunno man" / "Ultron said that" are rejected. This fixes both the silently-dropped follow-up command (the old path stamped a constant 0.75 < the 0.80 bar) and the blunt wake-word bypass — the wake word is now the strongest *feature*, not a hard short-circuit
