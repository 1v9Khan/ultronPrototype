...
10. **`docs/vision_roadmap.md`** — the long-horizon vision list (8 phases: inference substrate → **game intelligence / the Valorant assistant** → post-training → portability → capability broker → Kenning identity/personas → platform surface → productization), each item with build approach, components, work breakdown, integration points and acceptance bar. Phase 2 opens with a **two-axis model** that governs every game-data feature: **footprint** (`F0` nothing runs on the game PC · `F1` OS file-share I/O only · `F2` a helper process of ours · `F3` game-process contact — NEVER, `BR-P1`) × **policy exposure** (`P0` past/own-account · `P1` own live session metadata · `P2` live info about other players — contested, default-OFF behind its own gate · `P3` play automation — NEVER). Each item declares an `(F, P)` pair; the axes are independent, so a zero-footprint design can still be P2. Live match context (2.3) is `F1/P1` via share-tailing the game's own log, with an `F0/P2` same-account-second-client option gated on an explicit decision. NOT a commitment — items become real only with a committed spec. `docs/ultron_1_0/` still governs what is being built NOW; `docs/canon/` governs HOW; `docs/ultron_1_0/CONSTRAINTS.md` governs what must never regress.
