...
10. **`docs/vision_roadmap.md`** — the long-horizon vision list (7 phases: inference substrate → post-training → portability → capability broker → Kenning identity/personas → platform surface → productization), each item with build approach, components, work breakdown, integration points and acceptance bar. NOT a commitment — items become real only with a committed spec. `docs/ultron_1_0/` still governs what is being built NOW; `docs/canon/` governs HOW; `docs/ultron_1_0/CONSTRAINTS.md` governs what must never regress.
