...
|------|------|---------|-------|-------------|
| 2026-06-18 | `ad15ded` (main) | **Pre-roll→VAD freeze fix + wake-word-only stand-down** (the "said it 8 times before he responded" live freeze). Symptom: the wake word fired EVERY time (scores 0.85–0.94) yet the turn was discarded as `loop:empty_capture` — Ultron stayed silent. Root cause: a command spoken immediately after "Ultron" ("Ultron say hello") lands in the COLD pre-roll (`ring.snapshot` → `chunks[0]`), captured BEFORE the live loop's first `get_chunk`; the live-loop VAD only ever saw the post-command SILENCE, so `speech_seen` stayed False and the capture bailed empty — DISCARDING the buffer that already held the command. **Fix 1 (`_capture_utterance`):** feed `chunks[0]` (the pre-roll) through the VAD before the live loop (`vad.process` self-slices arbitrary audio into 512-sample windows); if the pre-roll is speech, latch `speech_seen=True` / `speech_start=0` so the buffer is kept + transcribed. `chunks[0]` is never re-processed by the loop ⇒ no double-count; fail-open. **Fix 2 (router, `run()`):** now that the pre-roll is VAD-fed, a bare "Ultron" (no command) registers as speech and is captured + transcribed as just the wake word. Guard: if the RAW transcript is only a wake remnant/filler with nothing after it (`_WAKE_REMNANT_RE` match consumes the whole string), log `routing:wake_word_only` and `continue` (stand down). Checked on the RAW text BEFORE `normalize_command` — normalize does NOT empty a bare wake word (keeps "Ultron", mangles bare "Tron" → the callout "drone"), so an after-normalize check would miss it and fire a spurious turn. NOTE: 2 PRE-EXISTING relay-corpus failures remain (`test_value_swap_keeps_last_buy`, `test_say_to_delivers_literal_payload` — compound reported-directive payload extraction), unrelated to this fix (verified by stashing `orchestrator.py`); flagged for a separate relay-coherence pass. | 22 capture/orch + 245 addr/fw/corpus (2 pre-existing relay fails) | [project_prelaunch_hardening_2026_06_17.md](file:///C:/Users/alecf/.claude/projects/C--STC-ultronPrototype/memory/project_prelaunch_hardening_2026_06_17.md) |
