...
10. **`docs/vision_roadmap.md`** — the long-horizon vision list (8 phases: inference substrate → **game intelligence / the Valorant assistant** → post-training → portability → capability broker → Kenning identity/personas → platform surface → productization), each item with build approach, components, work breakdown, integration points and acceptance bar. Phase 2 opens with the **data-source risk ladder** (GREEN post-match web APIs · AMBER local Riot-client lockfile reads, default-OFF and gated · RED memory/injection/input-automation, never) — every game-data feature declares its rung, and `BR-P1` governs. NOT a commitment — items become real only with a committed spec. `docs/ultron_1_0/` still governs what is being built NOW; `docs/canon/` governs HOW; `docs/ultron_1_0/CONSTRAINTS.md` governs what must never regress.
